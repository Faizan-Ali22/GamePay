/**
 * GamePay - Main Backend Server (Express.js + MSSQL)
...
// Connect to SQL Server database (with graceful fallback if DB is offline)
sql.connect(dbConfig)
  .then(p => {
    pool = p;
    console.log('✅ Connected to SQL Server database (gamestore) - Configured for Faizan Ali');
  })
  .catch(err => {
    console.warn('⚠️ SQL Server Connection Warning:', err.message);
    console.warn('⚠️ GamePay backend is running in Demo Fallback Mode until SQL Server is available.');
  });
  ...
  try {
    const result = await pool.request().query(`
      SELECT 
        ...
        oi.order_item_id,
        ISNULL(g.title, 'Deleted Game') AS game_title,
        oi.quantity,
        oi.price
      FROM orders o
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, userType: roleName, userId: user.user_id, username: user.username });
  } catch (err) {
...
app.get('/test', (req, res) => {
  res.send('GamePay Server is working! (Configured for Faizan Ali)');
});
...
// Start the server
app.listen(port, () => {
  console.log(`🚀 GamePay Server running at http://localhost:${port}`);
  console.log(`👤 Custom Backend Owner: Faizan Ali`);
});
